...
public class ObjetoArrayInt extends ObjetoArray{
	private Tipado tipo;

	public ObjetoArrayInt(String nombre, int ambito, int lengths, Valor valor) {
		super(nombre, ambito, lengths, valor);
		this.tipo = Tipado.ARRAY_INT;
	}
	...
	public Objeto metodosInstancia(TablaSimbolos tabla, Metodos metodo, Vector<Objeto> parametros) {
		...
		switch (metodo) {
			...
			case PRINT: {
				Objeto temporal = new ObjetoInt(tabla.getTemporal(), tabla.getAmbito().getBloque(), Valor.TEMPORAL);
