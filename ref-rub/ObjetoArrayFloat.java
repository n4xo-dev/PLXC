...
public class ObjetoArrayFloat extends ObjetoArray{
	private Tipado tipo;

	public ObjetoArrayFloat(String nombre, int ambito, int lengths, Valor valor) {
		super(nombre, ambito, lengths, valor);
		this.tipo = Tipado.ARRAY_FLOAT;
	}
	...
	public Objeto metodosInstancia(TablaSimbolos tabla, Metodos metodo, Vector<Objeto> parametros) {
		...
		switch (metodo) {
			...
			case PRINT: {
				Objeto temporal = new ObjetoFloat(tabla.getTemporal(), tabla.getAmbito().getBloque(), Valor.TEMPORAL);
			...
			case ASIGNACION_INDICE: {
				...
				switch (expresion.getTipo()) {
					case FLOAT:
						result = this;
						PLXC.out.println("   "+this+"["+indice+"]"+" = "+expresion+";");
						break;
					case INT:
						Objeto cast = new ObjetoFloat(tabla.getTemporal(), tabla.getAmbito().getBloque(), Valor.TEMPORAL);
						PLXC.out.println("   "+cast+" = (float) "+expresion+";");
						result = this;
						PLXC.out.println("   "+this+"["+indice+"]"+" = "+cast+";");
						break;
					default:
						PLXC.out.println("# ERROR: Se esta asignando un valor "+expresion.getTipo().name()+" a una variable "+this.getTipo().name());
						PLXC.out.println("   error;");
