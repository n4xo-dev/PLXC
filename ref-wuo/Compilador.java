...
public class Compilador {
  ...
  private static void printMatriz(String k, int length, String type) {
    ...
    for (int x = 0; x < length; x++) {
      ...
      System.out.println("print" +
          (type.equalsIgnoreCase("char") ? "c " : " ") + aux + " ;");
    }
  }
  ...
  public static String asig(String k, String v, String range, boolean cx) {
    ...
    String clave = k;
    if (!range.equals("-1") && !range.equals("char")) { // si es una variable array...
      clave = k + "[" + range + "]";
    }
    boolean isVar = false;
    isVar = v.matches("[a-zA-Z][a-zA-Z0-9]*");
    if (isVar) { // es una variable
      if (isArray(k).getLength() != -1 &&
          isArray(v).getLength() != -1) {
  ...
  public static String operacion(String var1, String var2, String op) {
    ...
    // if(op.equals("+")||op.equals("-")||op.equals("*")||op.equals("/"))op="r";
  ...
  public static String declaracion(String type, String k, String v) {
    ...
    if (!k.contains("$") && found && (aux + 1) == contextActual) { // encontrada en mismo contexto
  ...
  public static String declaracionArray(String type, String k, String length, String fila, String columna,
      String profundidad) {
    boolean found = false;
    int aux = contextActual;
    while (aux >= 0 && !found) {
    ...
    if (found && (aux + 1) == contextActual) { // encontrada en mismo contexto
      error("variable \"" + k + "\" ya declarada.");
      return null;
    } else { // no encontrada o encontrada en otro contexto -> la declaramos
      if (variables.containsKey(contextActual)) {
      ...
      if (!profundidad.equals(""))
        System.out.println("$" + k + "_prof_size = " +
            profundidad + " ;");
    }
    return k;
  }
  ...
  public static String checkDeclaracion(String k) {
    ...
    if (found) {
      ...
      if (antes && variables.containsKey(contextActual) && variables.get(contextActual).contains(k)) { // encontrada
                                                                                                       // antes y ahora
                                                                                                       // -> nuevo
                                                                                                       // nombre
        boolean check = false;
  ...
  public static boolean isType(String var, String type) {
    ...
    for (int context : variables.keySet()) {
      for (Tag t : variables.get(context)) {
        if (t.getA().equalsIgnoreCase(type) &&
            t.getB().equals(var)) {
          found = true;
        }
      }
  ...
  public static void imprimeResultados() {
    System.out.println("\n\n# ********VARIABLES********");
    for (ArrayList<Tag> vars : variables.values()) {
      for (Tag t : vars) {
  ...
  public static void checkRangoMultiple(String k, String f, String c,
      String p) {
  ...
  public static String inicializaArray(String k, ArrayList<String> values, boolean nuevo) {
    ...
    if (length < values.size()) {
      error("Has intentado inicializar un array de longitud " +
          length + " con " + values.size() + " valores");
    } else {
      ...
      for (int x = 0; x < values.size(); x++) {
        System.out.println(var_aux2 + " = " + var_aux1 + "[" +
            x + "] ;");
  ...
  public static boolean checkAllTypes(String type, ArrayList<String> values) {
    if (type.equalsIgnoreCase("int")) { // comprobamos si todos son
      for (String v : values) {
